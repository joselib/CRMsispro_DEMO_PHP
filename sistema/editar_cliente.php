...
if (!empty($_POST)) {
  ...
  if (empty($_POST['nombre']) || empty($_POST['telefono']) || empty($_POST['direccion'])) {
    ...
    if (is_numeric($cc) and $cc != 0) {
      ...
      $query = mysqli_query($conexion, "SELECT * FROM cliente where (cc = '$cc' AND idcliente != $idcliente)");
      $result = mysqli_fetch_array($query);
      $resul = mysqli_num_rows($query);
    }

    if ($resul >= 1) {
      $alert = '<p class"error">El cc ya existe</p>';
    } else {
      if ($cc == '') {
        $cc = 0;
      }
      $sql_update = mysqli_query($conexion, "UPDATE cliente SET cc = $cc, nombre = '$nombre' , telefono = '$telefono', direccion = '$direccion' WHERE idcliente = $idcliente");
            ...
            <div class="col-lg-6 m-auto">
              ...
              <form class="" action="" method="post">
                ...
                <div class="form-group">
                  ...
                  <input type="number" placeholder="Ingrese cc" name="cc" id="cc" class="form-control" value="<?php echo $cc; ?>">
                </div>
                <div class="form-group">
